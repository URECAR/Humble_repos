...
class RobotiqHandeDriver(Node):
    ...
    def _init_ros_interfaces(self):
        """ROS2 인터페이스 초기화"""
        # 퍼블리셔들
        self.status_pub = self.create_publisher(Robotiq2FGripperStatus, 'robotiq_hande/status', 10)
        ...
        # 편의용 퍼블리셔들
        self.width_pub = self.create_publisher(Float64, 'robotiq_hande/width', 10)
        self.object_pub = self.create_publisher(Bool, 'robotiq_hande/object_detected', 10)
        self.moving_pub = self.create_publisher(Bool, 'robotiq_hande/moving', 10)
        self.position_pub = self.create_publisher(UInt8, 'robotiq_hande/position', 10)
